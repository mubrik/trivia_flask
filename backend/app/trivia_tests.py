...
#       # create all tables
#       # this function is already called in trivia init, leaving here cause might be dependent for test
#       self.db.create_all()
...
#     new_id = result_new.get_json()['question']['id']
#     # then try deleteing it
#     result = self.client().delete(f'/api/questions/{new_id}')
#     # checks
#     self.assertEqual(result.status_code, 200)
#     self.assertEqual(result.get_json()['success'], True)
#     self.assertEqual(result.get_json()['id'], new_id)
...
#     self.client().delete(f'/api/categories/{new_id}/delete')
...
#     new_id = result_new.get_json()['category']['id']
